...
void mind_attack_start(void* payload) {
...
void mind_attack_trigger(void* payload) {
    auto ability = id_ptr<Ability>((u64) payload);
...
void mind_attack_targeting(void* payload) {
    auto ability = id_ptr<Ability>((u64) payload);
...
void build_mind_mage(Scene* scene, entt::entity entity, const LizardPrefab& lizard_prefab) {
    ...
    liz.basic_ability = make_ability(scene, "Mind Basic");
    liz.basic_ability->caster = entity;
    liz.basic_ability->pre_trigger_time = Stat(1.0f);
    liz.basic_ability->post_trigger_time = Stat(1.0f);
    liz.basic_ability->cooldown_time = Stat(1.0f);
    liz.basic_ability->start_callback = mind_attack_start;
    liz.basic_ability->start_payload = (void*) liz.basic_ability.id;
    liz.basic_ability->trigger_callback = mind_attack_trigger;
    liz.basic_ability->trigger_payload = (void*) liz.basic_ability.id;
    liz.basic_ability->targeting_callback = mind_attack_targeting;
