...
void water_attack_start(void* payload) {
...
void water_attack_trigger(void* payload) {
    auto ability = id_ptr<Ability>((u64) payload);
...
void water_attack_targeting(void* payload) {
    auto ability = id_ptr<Ability>((u64) payload);
...
void build_water_mage(Scene* scene, entt::entity entity, const LizardPrefab& lizard_prefab) {
    ...
    liz.basic_ability = make_ability(scene, "Water Basic");
    ...
    liz.basic_ability->start_callback = water_attack_start;
    liz.basic_ability->start_payload = (void*) liz.basic_ability.id;
    liz.basic_ability->trigger_callback = water_attack_trigger;
    liz.basic_ability->trigger_payload = (void*) liz.basic_ability.id;
    liz.basic_ability->targeting_callback = water_attack_targeting;
