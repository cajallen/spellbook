...
bool DragMat3(const string& name, m33* matrix, f32 speed, const string& format);
bool DragMat4(const string& name, m44* matrix, f32 speed, const string& format);
...
void PathSelect(const string& hint, fs::path* out, const fs::path& base_folder, FileType file_type, bool open_subdirectories = true, const std::function<void(const fs::path&)>& context_callback = {});
void PathSelect(const string& hint, string* out, const string& base_folder, FileType file_type, bool open_subdirectories = true, const std::function<void(const fs::path&)>& context_callback = {});
